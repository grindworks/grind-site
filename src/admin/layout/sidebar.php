...
<body class="flex flex-col h-[100dvh] antialiased" :class="searchOpen ? 'overflow-hidden' : ''"
  ...
  <div class="relative flex flex-1 overflow-hidden" x-data="{ sidebarOpen: false }" x-init="$watch('sidebarOpen', val => window.toggleScrollLock(val))" @resize.window="if(window.innerWidth >= 768 && sidebarOpen) { sidebarOpen = false; }">
