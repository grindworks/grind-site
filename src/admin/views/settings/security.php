    ...
    <div class="block">
      ...
      <div class="flex gap-2" x-data="{ showPass: false }">
        ...
        <button type="button" @click="
            if(!user || !pass) { window.showToast(<?= htmlspecialchars(json_encode(_t('username_and_password_required')), ENT_QUOTES) ?>, 'error'); return; }
  ...
  <div x-show="result" class="mt-4" x-cloak>
    ...
    <div class="flex sm:flex-row flex-col gap-2">
      <div class="flex flex-1 w-full">
        ...
        <button @click="navigator.clipboard.writeText(result); window.showToast(<?= htmlspecialchars(json_encode(_t('copied')), ENT_QUOTES) ?>)" class="bg-theme-bg hover:bg-theme-surface px-3 border border-l-0 border-theme-border rounded-r-theme text-theme-text text-xs whitespace-nowrap transition-colors"><?= _t('btn_copy') ?></button>
      </div>
      <button type="button" @click="
          if(!confirm(<?= htmlspecialchars(json_encode(_t('confirm_save_htpasswd')), ENT_QUOTES) ?>)) return;
          fetch('settings.php?tab=security', {
              method: 'POST',
              headers: {'Content-Type': 'application/x-www-form-urlencoded'},
          ...
          .then(r => r.json())
          .then(data => {
              if(data.success) window.showToast(<?= htmlspecialchars(json_encode(_t('msg_saved')), ENT_QUOTES) ?>);
              else window.showToast(data.error, 'error');
          });
