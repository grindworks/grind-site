...
<div class="relative flex lg:flex-row flex-col gap-8"
  x-effect="document.body.style.overflow = (mobileFormOpen || deleteModalOpen) ? 'hidden' : ''"
  x-data='{
      ...
      <form method="post" @submit="setTimeout(() => isSubmitting = true, 10)">
        ...
        <div class="mb-4">
          ...
          <div class="flex">
            ...
            <input type="text" name="slug" value="<?= h($edit_data['slug']) ?>" class="rounded-l-none font-mono text-sm form-control placeholder-theme-text/30" placeholder="<?= _t('ph_auto_generate') ?>" <?= ($edit_data['slug'] === 'uncategorized') ? 'readonly' : '' ?>>
          </div>
        </div>
        ...
        <div class="flex gap-3">
          <?php if ($edit_id): ?>
            <a href="categories.php" class="flex-1 py-2.5 rounded-theme text-sm text-center btn-secondary"><?= _t('cancel') ?></a>
          <?php else: ?>
