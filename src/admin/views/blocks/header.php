...
<div class="flex items-center gap-3" x-init="if(!block.data.level) block.data.level = 'h2'; if(typeof block.data.text === 'undefined') block.data.text = '';">
  ...
  <!-- Header text -->
  <input type="text" x-model="block.data.text" :id="'block-' + block.id + '-text'" @keydown.enter.prevent class="bg-transparent px-2 py-1 border-none focus:ring-0 w-full font-bold text-theme-text text-2xl leading-tight transition-colors placeholder-theme-text/30" placeholder="<?= h(_t('ph_enter_header')) ?>">
</div>
