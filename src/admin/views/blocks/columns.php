...
<div class="space-y-3" x-init="if(!block.data.ratio) block.data.ratio = '1-1'">
  ...
  <div class="gap-4 grid grid-cols-1" :class="{
    ...
    <div class="bg-theme-bg/40 p-4 border border-theme-border border-dashed rounded-theme">
      ...
      <textarea x-model="block.data.leftText" rows="4" class="bg-transparent p-1 border-none focus:ring-0 w-full text-theme-text text-sm placeholder-theme-text/30" placeholder="<?= _t('ph_enter_text') ?>"></textarea>
    </div>
    <div class="bg-theme-bg/40 p-4 border border-theme-border border-dashed rounded-theme">
      <div class="opacity-50 mb-1 font-bold text-[10px] text-theme-text"><?= _t('col_right') ?></div>
      <textarea x-model="block.data.rightText" rows="4" class="bg-transparent p-1 border-none focus:ring-0 w-full text-theme-text text-sm placeholder-theme-text/30" placeholder="<?= _t('ph_enter_text') ?>"></textarea>
    </div>
  </div>
</div>
