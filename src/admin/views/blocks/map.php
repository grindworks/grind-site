...
<div class="bg-theme-bg/40 p-4 border border-theme-border rounded-theme">
  ...
  <div x-show="block.data.code" class="bg-theme-bg mt-2 border border-theme-border rounded-theme aspect-video overflow-hidden">
    <template x-if="block.data.code && block.data.code.match(/<iframe\s+[^>]*src=[\x22\x27](https:\/\/(?:www\.)?(?:google\.com\/maps|openstreetmap\.org)[^\x22\x27]+)[\x22\x27]/i)">
      <iframe :src="block.data.code.match(/<iframe\s+[^>]*src=[\x22\x27](https:\/\/(?:www\.)?(?:google\.com\/maps|openstreetmap\.org)[^\x22\x27]+)[\x22\x27]/i)[1]" width="100%" height="100%" style="border:0;" allowfullscreen="" loading="lazy" class="w-full h-full"></iframe>
    </template>
    <template x-if="!(block.data.code && block.data.code.match(/<iframe\s+[^>]*src=[\x22\x27](https:\/\/(?:www\.)?(?:google\.com\/maps|openstreetmap\.org)[^\x22\x27]+)[\x22\x27]/i))">
