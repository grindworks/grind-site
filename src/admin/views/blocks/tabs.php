...
<div class="space-y-3" x-init="if(!block.data.items) block.data.items =[{id: Math.random().toString(36).substr(2, 9), title:'Tab 1', content:''}]; block.data.items.forEach(i => { if(!i.id) i.id = Math.random().toString(36).substr(2, 9); })">
    <!-- Loop tabs -->
        ...
        <div class="group/item relative bg-theme-bg/40 mb-2 p-4 border border-theme-border rounded-theme">
            ...
            <div class="space-y-3 pr-8">
                ...
                <div>
                    ...
                    <input type="text" x-model="item.title" class="w-full font-bold form-control-sm" placeholder="<?= _t('ph_tab_title') ?>">
                </div>
                <!-- Content input -->
                <div>
                    <label class="block opacity-50 mb-1 font-bold text-[10px] text-theme-text"><?= _t('lbl_content') ?></label>
                    <textarea x-model="item.content" rows="4" class="w-full text-xs form-control-sm leading-relaxed" placeholder="<?= _t('ph_tab_content') ?>"></textarea>
                </div>
            </div>
        </div>
    </template>
    ...
    <!-- Add tab button -->
    <button type="button" @click="block.data.items.push({id: Math.random().toString(36).substr(2, 9), title:'Tab ' + (block.data.items.length + 1), content:''}); $nextTick(() => { $el.closest('.space-y-3').querySelectorAll('input[type=text]')[block.data.items.length-1]?.focus() })" class="py-2 border-2 hover:border-theme-primary/50 border-dashed w-full text-xs btn-secondary">+ <?= _t('btn_add_tab') ?></button>
</div>
