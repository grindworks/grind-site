  ...
  <div class="flex sm:flex-row flex-col items-start gap-4">
    ...
    <div class="flex-1 space-y-3 w-full">
      <textarea x-model="block.data.comment" :id="'block-' + block.id + '-comment'" rows="2"
        ...
        class="w-full text-sm form-control-sm resize-none overflow-hidden" placeholder="<?= h(_t('ph_comment')) ?>"></textarea>
      <div class="flex sm:flex-row flex-col gap-3">
        <div class="flex-1">
          <input type="text" x-model="block.data.name" :id="'block-' + block.id + '-name'" class="w-full font-bold text-xs form-control-sm" placeholder="<?= h(_t('ph_name')) ?>">
        </div>
        <div class="flex-1">
          <input type="text" x-model="block.data.role" :id="'block-' + block.id + '-role'" class="w-full text-xs form-control-sm" placeholder="<?= h(_t('ph_role')) ?>">
        </div>
      </div>
    </div>
  </div>
