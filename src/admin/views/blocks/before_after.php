  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
    ...
    <div class="space-y-2">
      <div class="flex justify-between items-center">
        ...
        <input type="text" x-model="block.data.beforeLabel" class="w-24 text-xs form-control-sm text-center" placeholder="Before">
      </div>
      ...
      <div class="flex gap-2">
        <input type="text" x-model="block.data.beforeUrl" @blur="block.data.beforeUrl = normalizeUrl(block.data.beforeUrl)" class="flex-1 font-mono text-xs form-control-sm" placeholder="URL">
        <button type="button" @click="openMediaLibrary(index, null, 'beforeUrl')" class="px-2 py-1 text-xs btn-secondary shrink-0" title="<?= h(_t('btn_select_library')) ?>">
        ...
        <label class="px-2 py-1 text-xs cursor-pointer btn-secondary shrink-0" :class="{'opacity-50 cursor-not-allowed': isUploadingBefore}">
          <input type="file" class="hidden" accept="image/*" @change="isUploadingBefore = true; await uploadImage($event, index, 'beforeUrl'); isUploadingBefore = false" :disabled="isUploadingBefore">
    ...
    <div class="space-y-2">
      <div class="flex justify-between items-center">
        ...
        <input type="text" x-model="block.data.afterLabel" class="w-24 text-xs form-control-sm text-center" placeholder="After">
      </div>
      <div class="group relative bg-checker border border-theme-border rounded-theme aspect-video overflow-hidden">
      ...
      <div class="flex gap-2">
        <input type="text" x-model="block.data.afterUrl" @blur="block.data.afterUrl = normalizeUrl(block.data.afterUrl)" class="flex-1 font-mono text-xs form-control-sm" placeholder="URL">
        <button type="button" @click="openMediaLibrary(index, null, 'afterUrl')" class="px-2 py-1 text-xs btn-secondary shrink-0" title="<?= h(_t('btn_select_library')) ?>">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <use href="<?= grinds_asset_url('assets/img/sprite.svg') ?>#outline-photo"></use>
          </svg>
        </button>
        <label class="px-2 py-1 text-xs cursor-pointer btn-secondary shrink-0" :class="{'opacity-50 cursor-not-allowed': isUploadingAfter}">
          <input type="file" class="hidden" accept="image/*" @change="isUploadingAfter = true; await uploadImage($event, index, 'afterUrl'); isUploadingAfter = false" :disabled="isUploadingAfter">
