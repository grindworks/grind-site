...
<div class="bg-theme-bg/40 p-4 border border-theme-border rounded-theme transition-colors"
  ...
  <div class="flex flex-col gap-3">
    ...
    <div>
      ...
      <input type="text" x-model="block.data.title" :id="'block-' + block.id + '-title'" class="w-full font-bold form-control-sm" placeholder="<?= h(_t('ph_track_title')) ?>">
    </div>
    <div class="flex gap-2">
      <!-- Audio URL -->
      <input type="text" x-model="block.data.url" :id="'block-' + block.id + '-url'" @blur="block.data.url = normalizeUrl(block.data.url)" class="flex-1 font-mono text-xs form-control-sm" placeholder="<?= h(_t('ph_audio_url')) ?>">
      <!-- Open media library -->
