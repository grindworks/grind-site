...
?>
<div class="space-y-4"
  x-init="if(!block.data.items) block.data.items = [{id: generateId(), date:'', title:'', content:''}]; block.data.items.forEach(i => { if(!i.id) i.id = generateId(); })"
  ...
  <template x-for="(item, i) in block.data.items" :key="item.id">
    <div class="flex gap-4">
      ...
      <div class="relative flex-grow space-y-2 bg-theme-bg/40 mb-4 p-4 pr-8 border border-theme-border rounded-theme">
        <!-- Delete item -->
        <button type="button" @click="if(!item.date && !item.title && !item.content || confirm(window.grindsTranslations?.confirm_delete || 'Are you sure?')) block.data.items.splice(i, 1)"
  ...
  <!-- Add event button -->
  <button type="button"
    @click="block.data.items.push({id: generateId(), date:'', title:'', content:''})"
