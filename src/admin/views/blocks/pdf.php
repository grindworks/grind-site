...
<div class="bg-theme-bg/40 p-4 border border-theme-border rounded-theme" x-data="{ isUploading: false }">
  <div class="flex flex-col gap-3">
    ...
    <div>
      ...
      <input type="text" x-model="block.data.title" class="w-full font-bold form-control-sm" placeholder="<?= _t('ph_doc_title') ?>">
    </div>
    <div class="flex gap-2">
      <!-- PDF URL -->
      <input type="text" x-model="block.data.url" class="flex-1 font-mono text-xs form-control-sm" placeholder="<?= _t('ph_pdf_url') ?>">
      <!-- Open media library -->
