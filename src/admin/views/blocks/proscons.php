...
<div class="gap-4 grid grid-cols-1 sm:grid-cols-2" x-init="if(!block.data.pros_items) block.data.pros_items = ['']; if(!block.data.cons_items) block.data.cons_items = [''];" x-data="{ styles: <?= htmlspecialchars(json_encode($proscons_styles), ENT_QUOTES) ?> }">
  ...
  <div class="p-4 border rounded-theme" :class="styles.pros.class">
    <div class="flex items-center gap-2 mb-2 pb-2 border-theme-success/30 border-b">
      ...
      <input type="text" x-model="block.data.pros_title" :id="'block-' + block.id + '-pros_title'" class="bg-transparent p-1 focus:outline-none w-full font-bold text-inherit text-sm placeholder-current/50" placeholder="<?= h(_t('lbl_pros_title')) ?>">
    </div>
    <div class="space-y-2">
      <!-- Loop pros -->
      <template x-for="(item, i) in block.data.pros_items" :key="i">
        <div class="group/item flex items-center gap-2">
          ...
          <input type="text" x-model="block.data.pros_items[i]" :id="'block-' + block.id + '-pros_item-' + i" class="flex-1 border border-theme-success/30 focus:border-theme-success form-control-sm" placeholder="<?= h(_t('ph_pros_item')) ?>"
  ...
  <div class="p-4 border rounded-theme" :class="styles.cons.class">
    <div class="flex items-center gap-2 mb-2 pb-2 border-theme-danger/30 border-b">
      ...
      <input type="text" x-model="block.data.cons_title" :id="'block-' + block.id + '-cons_title'" class="bg-transparent p-1 focus:outline-none w-full font-bold text-inherit text-sm placeholder-current/50" placeholder="<?= h(_t('lbl_cons_title')) ?>">
    </div>
    <div class="space-y-2">
      <!-- Loop cons -->
      <template x-for="(item, i) in block.data.cons_items" :key="i">
        <div class="group/item flex items-center gap-2">
          ...
          <input type="text" x-model="block.data.cons_items[i]" :id="'block-' + block.id + '-cons_item-' + i" class="flex-1 border border-theme-danger/30 focus:border-theme-danger form-control-sm" placeholder="<?= h(_t('ph_cons_item')) ?>"
