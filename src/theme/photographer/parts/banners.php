    ...
    <div class="banner-area mb-8 grid gap-6 banner-pos-<?= h($position) ?> <?= $extraClass ?>">
        <?php foreach ($position_banners as $b): ?>
            ...
            if ($bType === 'html') {
                ...
                $anchorClass = ($b['anchor_class'] ?? 'block') . ' group relative overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1';
                ...
                else echo '<div class="group block relative shadow-lg overflow-hidden" style="' . $anchorStyle . '">';
