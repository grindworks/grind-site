...
<article class="bg-white border-2 border-slate-900 shadow-sharp mb-16 overflow-hidden">
  <header class="p-8 md:p-12 pb-0">
    <!-- Breadcrumbs -->
    <div class="mb-8">
      <?= get_breadcrumb_html([
    ...
    <div class="flex flex-wrap items-center gap-4 mb-4 text-gray-600 text-sm">
      ...
      <?php if (!empty($pageData['post']['show_category']) && $pageData['post']['category_name']): ?>
        <a href="<?= h(resolve_url('category/' . $pageData['post']['category_slug'])) ?>"
          class="bg-brand-50 border-2 border-slate-900 hover:bg-brand-500 hover:text-white px-3 py-1 font-bold text-slate-900 text-xs transition-colors shadow-[2px_2px_0_0_rgba(15,23,42,1)] hover:-translate-y-0.5 uppercase tracking-widest">
      ...
      <?php if (!empty($pageData['post']['show_date'])): ?>
        ...
        $pubTs = strtotime((string)$pubDateStr) ?: time();
        $modTs = strtotime((string)$modDateStr) ?: $pubTs;
        ?>
      ...
      <?php if (!empty($_SESSION['admin_logged_in']) && !isset($_GET['preview'])): ?>
        ...
        <a href="<?= h(resolve_url('admin/posts.php?action=edit&id=' . $pageData['post']['id'])) ?>" target="_blank"
          class="group flex items-center gap-1 bg-gray-800 hover:bg-grinds-red shadow-sm ml-auto px-3 py-1 rounded-full font-bold text-white text-xs transition"
    ...
    <?php if (!$hasHeroTitle): ?>
      <!-- Render post title -->
      <h1
        class="mb-8 font-heading font-extrabold text-slate-900 text-4xl md:text-5xl lg:text-7xl leading-[1.1] tracking-tight">
    ...
    <?php if ($pageData['post']['thumbnail'] && !$hasHero): ?>
      <div class="-mx-6 md:-mx-10 mb-8">
        <?= get_image_html(resolve_url($pageData['post']['thumbnail']), ['alt' => h($pageData['post']['title']), 'class' => 'w-full h-auto max-h-[500px] object-cover', 'loading' => 'eager', 'fetchpriority' => 'high']) ?>
      </div>
    <?php
    endif; ?>
  </header>
  ...
  <div class="px-6 md:px-10 pb-10">
    <!-- Render post content -->
    <div id="post-content" class="mx-auto text-slate-800 leading-relaxed text-lg max-w-4xl">
      ...
      ?>
          <div
            ...
            <?php if ($pImage): ?>
              <img src="<?= h($pImage) ?>" alt="<?= h($pName) ?>"
                class="w-24 h-24 border-2 border-slate-900 object-cover shadow-sharp shrink-0" loading="lazy">
            <?php
            else: ?>
              <div
                class="w-24 h-24 border-2 border-slate-900 bg-brand-50 flex items-center justify-center text-slate-400 text-3xl shrink-0">
                👤</div>
            <?php
            endif; ?>
            <div class="text-center sm:text-left">
              <div class="text-sm font-bold text-slate-400 uppercase tracking-widest mb-2">Written by</div>
              <h3 class="text-2xl font-heading font-extrabold text-slate-900 mb-3">
                <?= h($pName) ?>
              </h3>
              ...
              ?>
                <div class="mt-4 flex flex-wrap justify-center sm:justify-start gap-4">
                  <?php foreach ($socialLinks as $link): ?>
                    <a href="<?= h($link) ?>" target="_blank" rel="noopener noreferrer me"
                      class="text-xs font-bold text-grinds-red hover:underline flex items-center gap-1">
  ...
  <footer class="bg-brand-50 px-8 md:px-12 py-10 border-slate-900 border-t-2">
    <!-- Render tags if available -->
    <?php if (!empty($pageData['tags'])): ?>
      <div class="flex flex-wrap gap-3 mb-6">
        <?php foreach ($pageData['tags'] as $tag): ?>
          <a href="<?= h(resolve_url('tag/' . $tag['slug'])) ?>"
            class="bg-white px-4 py-1.5 border-2 border-slate-900 text-slate-900 hover:bg-slate-900 hover:text-white font-bold text-sm uppercase tracking-widest shadow-[2px_2px_0_0_rgba(15,23,42,1)] hover:-translate-y-0.5 transition-all">
            #
            <?= h($tag['name']) ?>
          </a>
        <?php
