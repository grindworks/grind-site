...
<article class="bg-white shadow-sm border border-corp-border rounded-lg overflow-hidden">
  ...
  <header class="p-8 border-gray-100 border-b">
    <div class="flex items-center gap-4 mb-4 text-gray-500 text-xs">
      <time><?= date('Y.m.d', strtotime($pageData['post']['published_at'])) ?></time>
      <?php if ($pageData['post']['category_name']): ?>
        <span class="bg-corp-light px-2 py-1 rounded font-bold text-corp-main">
  ...
  <footer class="bg-gray-50 px-8 py-6 border-gray-100 border-t">
    <?php
    $post = $pageData['post'] ?? [];
    if ((!isset($post['show_share_buttons']) || !empty($post['show_share_buttons'])) && function_exists('corporate_the_share_buttons')) {
      $shareUrl = resolve_url($post['slug']);
      $shareTitle = $post['title'];
      corporate_the_share_buttons($shareUrl, $shareTitle);
    }
    ?>
    <div class="flex justify-between items-center mt-6">
      <a href="<?= h(resolve_url('/')) ?>" class="font-bold text-corp-accent text-sm hover:underline">&larr; <?= theme_t('back_to_home') ?></a>
    </div>
  </footer>
</article>
