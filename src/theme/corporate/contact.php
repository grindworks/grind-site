...
<article class="bg-white shadow-sm border border-corp-border rounded-lg overflow-hidden">
  ...
  <div class="p-8 md:p-12">
    ...
    <?php if ($success): ?>
      <div class="bg-green-50 mb-6 px-6 py-8 border border-green-200 rounded text-green-700 text-center">
        <div class="flex justify-center mb-4">
          <svg class="w-12 h-12 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
          </svg>
        </div>
        <p class="mb-4 font-bold text-lg"><?= theme_t('contact_complete', 'Sent Successfully') ?></p>
        <p class="text-base leading-relaxed"><?= $success ?></p>
        <a href="<?= h(resolve_url('/')) ?>" class="inline-block mt-6 hover:text-green-800 text-sm underline font-bold"><?= theme_t('contact_back', 'Back to Home') ?></a>
      </div>
    <?php else: ?>
      ...
      <form method="post" action="?" class="space-y-6 max-w-2xl" onsubmit="
      ...
      ">
        ...
        <?php foreach ($formFields as $key => $field): ?>
          ...
          <div class="<?= h($field['width'] ?? 'w-full') ?>">
            <label class="block mb-2 font-bold text-gray-700 text-sm" for="field-<?= $key ?>">
              <?= h($field['label']) ?><?= $reqLabel ?>
            </label>

            <?php if ($field['type'] === 'textarea'): ?>
              <textarea name="<?= $key ?>" id="field-<?= $key ?>" rows="<?= $field['rows'] ?? 5 ?>" <?= !empty($field['required']) ? 'required' : '' ?> placeholder="<?= $ph ?>" class="px-4 py-3 border border-gray-300 focus:border-corp-accent rounded outline-none focus:ring-1 focus:ring-corp-accent w-full transition bg-white"></textarea>
            <?php elseif ($field['type'] === 'select'): ?>
              <select name="<?= $key ?>" id="field-<?= $key ?>" <?= !empty($field['required']) ? 'required' : '' ?> class="px-4 py-3 border border-gray-300 focus:border-corp-accent rounded outline-none focus:ring-1 focus:ring-corp-accent w-full transition bg-white cursor-pointer">
            ...
            <?php else: ?>
              <input type="<?= h($field['type']) ?>" name="<?= $key ?>" id="field-<?= $key ?>" value="<?= $val ?>" <?= !empty($field['required']) ? 'required' : '' ?> placeholder="<?= $ph ?>" class="px-4 py-3 border border-gray-300 focus:border-corp-accent rounded outline-none focus:ring-1 focus:ring-corp-accent w-full transition bg-white">
            <?php endif; ?>
          </div>
        <?php endforeach; ?>
        ...
        <div class="pt-4 pb-2">
          <label class="flex items-center gap-3 cursor-pointer">
            ...
            <input type="checkbox" name="privacy" value="1" required <?= $privacyChecked ?> class="w-5 h-5 text-corp-accent border-gray-300 rounded focus:ring-corp-accent transition bg-white">
            <span class="text-sm text-gray-700">
              <?= theme_t('I agree to the <a href="%s" target="_blank" class="text-corp-accent underline hover:no-underline">Privacy Policy</a>', h(resolve_url('/privacy-policy'))) ?>
            </span>
          </label>
        </div>
        ...
        <div class="pt-4">
          <?php if (empty($recipientEmail)): ?>
            <button type="button" disabled class="bg-gray-400 shadow px-10 py-4 rounded font-bold text-white cursor-not-allowed">
