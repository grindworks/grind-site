...
内部リンクのパス解決には `resolve_url()` を使用します。テーマ固有の画像やCSSなどの静的アセットには `grinds_theme_asset_url()` を使用することで、テーマ複製後のパス解決や default テーマへの自動フォールバックが機能します。
...
### カスタムフィールド (`theme.json`)

テーマディレクトリに `theme.json` を作成することで、独自のカスタムフィールドを定義できます。
定義したフィールドは、PHPを書くことなく記事編集画面のサイドバーに自動的に入力欄として追加されます。
...
テンプレート（例：`single.php`）で入力された値を取得・表示するには `get_post_meta()` を使用します。
...
### トップページに特定の固定ページを表示する

デフォルトのルーティングでは、トップページ（`/`）にアクセスすると `home.php` が読み込まれ、新着記事一覧が表示されます。
もし任意の固定ページ（例: スラグが `home-demo` のページ）の内容をトップページとして表示したい場合は、テーマの `home.php` を編集してデータベースから該当ページを取得し、レンダリングするコードを追加してください。
