...
<!-- TODO: Replace this URL with actual GIF of Smart Paste in action -->
...
GrindSite is the AI-Native CMS, meticulously engineered for the modern content creator's workflow. It eliminates tedious setup and maintenance, allowing you to move from idea to publication at the speed of thought.

Paste content directly from **ChatGPT, Claude, or Gemini**. Drop images from **Midjourney or DALL-E**. GrindSite intelligently structures everything into clean, editable blocks. Once published, your site is automatically optimized for AI search engines (`llms.txt`), ensuring your content is discovered and understood.

### ✨ Key Features

- **AI-First Workflow:**
  - **Smart Paste:** Paste Markdown, rich text, or even raw HTML from any source (ChatGPT, Claude, Gemini, Notion). GrindSite instantly converts it into structured blocks.
...
GrindSiteは、現代のコンテンツ制作者のワークフローのために設計された「AIネイティブCMS」です。
面倒なセットアップや保守作業を一切不要にし、アイデアから公開までを思考のスピードで実現します。

**ChatGPT, Claude, Gemini** からのコンテンツを直接ペースト。**Midjourney, DALL-E** の画像をドラッグ＆ドロップ。GrindSiteはそれらを即座にクリーンで編集可能なブロックへと変換します。公開されたサイトはAI検索エンジンに最適化され（`llms.txt`）、あなたのコンテンツが発見され、正しく理解されることを保証します。
