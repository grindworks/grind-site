...
[![Version](https://img.shields.io/badge/version-1.7.0-blue.svg)](https://github.com/grindworks/grind-site/releases)
...
- **Smart Paste:** Copy-paste Markdown from ChatGPT/Claude, or directly paste images from your clipboard into the editor.
- **Post Revisions:** Never lose your work. Automatically saves a history of your post edits.
- **Custom Fields (`theme.json`):** Define custom fields directly in your theme without writing PHP.
- **Contact Form Management:** Fully customizable contact forms with dynamic subject dropdowns, auto-reply emails, automatic `Reply-To` routing, and **built-in anti-spam** (honeypot & time-trap).
- **Content Protection:** Secure specific blocks of your content using the built-in Password Protect block with AES-256-GCM encryption.
- **AI & LLM Ready:** Built-in `llms.txt` and `llms-full.txt` (Markdown archive) generation for RAG and AI engines.
- **Markdown Export:** Export your entire site content as clean Markdown files, perfect for AI analysis or migrating.
...
- **API & Plugins:** Built-in REST API (`/api/`) and extensible Plugin/Hook system (`/plugins/`). Manage and toggle plugins directly from the admin panel.
...
GrindSite includes a built-in Email OTP (One-Time Password) plugin for enhanced security.
To enable 2FA:
...
- **スマートペースト:** ChatGPT の Markdown をコピペしたり、クリップボードの画像を直接ペーストしてアップロードできます。
- **記事リビジョン機能:** 編集履歴を自動保存し、いつでも過去のバージョンへシームレスに復元できます。
- **カスタムフィールド:** `theme.json` で定義するだけで、PHPを書かずに専用の入力項目を作成可能です。
- **お問い合わせフォーム管理:** 宛先や件名の選択肢、自動返信メールなどの柔軟な設定に加え、**強力なスパム対策（ハニーポット・タイムトラップ）**を標準搭載。
- **コンテンツ保護:** パスワード保護ブロック（AES-256-GCM暗号化）を使用して、記事内の特定のコンテンツをセキュアに隠すことができます。
- **AI・LLM 完全対応:** RAGやAIクローラー向けに、サイト全体をMarkdownで提供する `llms.txt` および `llms-full.txt` を動的生成。
- **Markdown一括エクスポート:** サイト全コンテンツをクリーンなMarkdownとして一括書き出し。AI分析や他システムへの移行に活用できます。
- **マルチテーマ対応:** **Tailwind CSS**（デフォルト）と **Bootstrap 5** テーマを同梱。カスタマイズも容易です。
- **サイト管理:** メニュー、ウィジェット、バナー、カテゴリ、タグなどを管理画面から自由に設定可能。
- **API ＆ プラグイン:** ヘッドレスCMSとして使えるREST API (`/api/`) と、機能を安全に拡張できるフック・プラグインシステムを搭載。プラグインは管理画面からワンクリックで有効・無効を切り替え可能です。
- **システムツール:** リンク切れチェック、不要ファイル検出、DB最適化、移行用パッケージ作成機能を搭載。
- **高速動作:** 余計な機能がないため、非常に高速にページが表示されます。
- **高い移植性:** ローカル（MAMP 等）で作って本番にアップロードするだけの簡単運用。
- **静的サイト書き出し (SSG):** サイト全体を純粋なHTMLとして書き出し可能。新たにバックグラウンドキューシステムを備え、大規模サイトの静的化にも対応しました。
...
GrindSiteには、セキュリティを強化するための「メールOTP（ワンタイムパスワード）プラグイン」が標準で同梱されています。
有効化する手順：
