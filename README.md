...
[![Version](https://img.shields.io/badge/version-1.0.3-blue.svg)](https://github.com/grindworks/grind-site/releases)
...
| Feature              |   Free (Trial/Dev)   |  Early Bird ($49)   |     Pro ($149)      |   Agency ($399/yr)    |
| :------------------- | :------------------: | :-----------------: | :-----------------: | :-------------------: |
...
| **One-Click Update** |   ❌ (Manual only)   |    ✅ Available     |    ✅ Available     |     ✅ Available      |

🎉 **Special Launch Pricing!** (Limited to the first 1,000 users or 1st month)

- **[Early Bird License (Single Site)](#)** - **$49** (No support included)
- **[Pro License (Single Site)](#)** - **$149**
- **[Agency License (Unlimited)](#)** - **$399/year** (For freelancers & web agencies)

> **[NOTICE]**
> Commercial licenses will be available for purchase very soon!
> We are preparing to launch sales via **Polar.sh**, a developer-friendly payment platform. Until then, please feel free to evaluate the Free version locally.
...
3.  **Update:** For free users, download the latest version from GitHub and overwrite the core files manually. Pro users can use the "One-Click Update" feature.

### 🔌 Completely Offline / Intranet Usage

For strict GDPR compliance or closed intranet environments, GrindSite features a "Disable External Assets" setting.
While the basic admin UI files are bundled, if you want features like Math blocks, Code Highlighting, and Dashboard Charts to work completely offline, you need to manually download and place the following files into the `src/assets/js/vendor/` and `src/assets/css/vendor/` directories:
...
```apache
# RewriteBase /your/detected/path/
```
...
| 機能・サポート       |      Free (無料版)       | アーリーバード ($49)  |      Pro ($149)       | エージェンシー ($399/年) |
| :------------------- | :----------------------: | :-------------------: | :-------------------: | :----------------------: |
...
| **ワンクリック更新** |   ❌ (手動上書きのみ)    |      ✅ 利用可能      |      ✅ 利用可能      |       ✅ 利用可能        |

🎉 **リリース記念キャンペーン！** (※最初の1ヶ月間、または先着1,000名様限定)

- **[アーリーバード版 (シングルサイト)](#)** - **$49** (サポートなし)
- **[Pro版 (シングルサイト)](#)** - **$149**
- **[エージェンシー版 (無制限ドメイン)](#)** - **$399/年** (制作会社・フリーランス向け)

> **【お知らせ】**
> 商用ライセンスは **近日販売開始予定** です！
> 現在、開発者に優しい決済プラットフォーム **Polar.sh** での販売準備を進めております。販売開始までは、ローカル環境等で無料版をご自由にお試しください。
...
3.  **アップデート:** 無料版をお使いの場合、最新版の zip をダウンロードし、手動でファイルを上書きすることでアップデート可能です。

### 🔌 完全なオフライン（イントラネット）環境での利用

厳格なGDPR対応や閉鎖網（イントラネット）での利用向けに、「外部アセット無効化」設定を備えています。
管理画面の基本動作に必要なファイルは同梱されていますが、数式ブロック、コードハイライト、ダッシュボードのグラフ等も完全にオフラインで動作させるには、以下のファイルを公式サイトからダウンロードし、`src/assets/js/vendor/` および `src/assets/css/vendor/` ディレクトリに手動で配置してください。
